    ...
    internal sealed class ControlFlowGraph
    {
                ...
                foreach (var statement in block.Statements)
                {
                    switch (statement)
                    {
                        case BoundLabelStatement _:
                            ...
                            break;
                        case BoundGotoStatement _:
                        case BoundConditionalGotoStatement _:
                        case BoundReturnStatement _:
                            _statements.Add(statement);
                            StartBlock();
                            break;
                        case BoundVariableDeclaration _:
                        case BoundExpressionStatement _:
                            _statements.Add(statement);
                            break;
                        default:
                            throw new Exception($"Unexpected statement: {statement.GetType().Name}");
                    }
                }
        ...
        public sealed class GraphBuilder
        {
            ...
            private BoundExpression Negate(BoundExpression condition)
            {
                if (condition is BoundLiteralExpression literal)
                {
                    ...
                    return new BoundLiteralExpression(TypeSymbol.Bool, !value);
                }

                var op = BoundUnaryOperator.Bind(TokenKind.Bang, TypeSymbol.Bool);
        ...
        public static bool AllPathsReturn(BoundBlockStatement body)
        {
            ...
            foreach (var branch in graph.End.Incoming)
            {
                ...
                if (lastStatement == null || !(lastStatement is BoundReturnStatement))
                    return false;
            }
